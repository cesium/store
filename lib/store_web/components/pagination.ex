defmodule StoreWeb.Components.Pagination do
  @moduledoc false
      ...
      <nav class="mb-5 flex w-full items-center justify-between px-4">
        ...
        <div class="hidden lg:-mt-px lg:flex">
          <%= if max(1, @meta.current_page - 2) != 1 do %>
            <%= live_patch("1",
              to: build_query(1, @params),
              class: "inline-flex items-center px-4 pt-4 text-sm font-medium text-gray-500 hover:text-gray-700"
            ) %>
          ...
          <%= for page <- max(1, @meta.current_page - 2)..max(min(@meta.total_pages, @meta.current_page + 2), 1) do %>
            <%= if @meta.current_page == page do %>
              <%= live_patch("#{page}",
                to: build_query(page, @params),
                class: "text-secondary inline-flex items-center px-4 pt-4 text-sm font-medium"
              ) %>
            <% else %>
              <%= live_patch("#{page}",
                to: build_query(page, @params),
          ...
          <%= if min(@meta.total_pages, @meta.current_page + 2) != @meta.total_pages do %>
            ...
            <% end %>
            <%= live_patch("#{@meta.total_pages}",
              to: build_query(@meta.total_pages, @params),
              class: "inline-flex items-center px-4 pt-4 text-sm font-medium text-gray-500 hover:text-gray-700"
            ) %>
          <% end %>
        </div>
